...
import { blogRegistry } from "@/content/blogs"
...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { metadata, default: BlogContent } = await loader()
          ...
          <div className="container mx-auto max-w-4xl">
            ...
            <div className="flex flex-wrap gap-2">
              {metadata.tags.map((tag) => (
                <Badge key={tag} variant="secondary" className="bg-white/10 text-white border-white/20 hover:bg-white/20">
                  {tag}
                </Badge>
              ))}
            </div>
